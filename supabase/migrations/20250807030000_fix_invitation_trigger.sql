-- Fix the onboarding trigger to properly handle invitation acceptance
-- The issue is that invitation lookup can fail due to case sensitivity or timing issues
-- We should use the company_id from metadata when available (from invitation data)
...
DO $$ 
BEGIN
    RAISE LOG 'Fixed invitation trigger to properly handle company_id from metadata and case-insensitive email matching';
END $$;
