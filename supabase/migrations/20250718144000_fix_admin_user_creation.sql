-- Fix the onboarding trigger to handle admin_company_id from metadata
-- This ensures users created by admins are assigned to the correct company
...
BEGIN
    ...
    RAISE LOG 'User onboarding trigger called for user: %, role: %, company: %, is_admin: %, admin_company_id: %', 
              NEW.email, user_role, company_name_val, is_admin_signup, admin_company_id;
    ...
    ELSE
        ...
        ELSE
            ...
            IF current_user_company_id IS NULL THEN
                RAISE EXCEPTION 'Cannot determine company for user creation';
            END IF;
            ...
            RAISE LOG 'Regular user profile created for: % with company: %', NEW.id, current_user_company_id;
        END IF;
...
DO $$ 
BEGIN
    RAISE LOG 'Updated user onboarding trigger to handle admin_company_id from metadata';
END $$;
